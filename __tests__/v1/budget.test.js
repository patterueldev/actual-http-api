...
describe('Budget Module', () => {
  ...
  describe('Accounts Management', () => {
    ...
    it('should get accounts with balances when includeBalances=true', async () => {
      const mockAccounts = [
      ...
      const mockBalanceData = [
        ...
        { account: 'acc2', cleared: true, total: 50000 },
        { account: 'acc2', cleared: false, total: 0 }
      ];

      mockActualApi.q = jest.fn((table) => ({
      ...
      runAqlQuery
        ...
        .mockResolvedValueOnce({ data: mockBalanceData });
      ...
      expect(accounts[0].workingBalance).toBe(9500);
      expect(accounts[1].clearedBalance).toBe(50000);
      expect(accounts[1].unclearedBalance).toBe(0);
      expect(accounts[1].workingBalance).toBe(50000);
    });

    it('should exclude offbudget accounts when excludeOffbudget=true', async () => {
      const allAccounts = [
        { id: 'acc1', name: 'Checking', offbudget: false, closed: false },
        { id: 'acc2', name: 'Savings', offbudget: true, closed: false }
      ];
      ...
      runAqlQuery
        .mockImplementation((query) => {
          if (runAqlQuery.mock.calls.length === 1) {
    ...
    it('should exclude closed accounts when excludeClosed=true', async () => {
      const allAccounts = [
        { id: 'acc1', name: 'Checking', offbudget: false, closed: false },
        { id: 'acc2', name: 'Old Checking', offbudget: false, closed: true }
      ];
      const filteredAccounts = [
        { id: 'acc1', name: 'Checking', offbudget: false, closed: false }
      ];
    ...
    it('should calculate workingBalance correctly', async () => {
      const mockAccounts = [
        { id: 'acc1', name: 'Checking', offbudget: false, closed: false }
      ];
      const mockBalanceData = [
        { account: 'acc1', cleared: true, total: 15000 },
        { account: 'acc1', cleared: false, total: 3000 }
      ];
      ...
      expect(accounts[0].clearedBalance).toBe(15000);
      expect(accounts[0].unclearedBalance).toBe(3000);
      expect(accounts[0].workingBalance).toBe(18000);
    });

    it('should handle accounts with no transactions (zero balances)', async () => {
      const mockAccounts = [
        { id: 'acc1', name: 'Checking', offbudget: false, closed: false }
      ];
    ...
    it('should use backward compatibility when includeBalances is false', async () => {
      const accounts = await budget.getAccounts({ includeBalances: false });

      expect(runAqlQuery).not.toHaveBeenCalled();
      expect(mockActualApi.getAccounts).toHaveBeenCalled();
      expect(accounts).toHaveLength(1);
    });

    it('should use backward compatibility when includeBalances is not provided', async () => {
      const accounts = await budget.getAccounts();
